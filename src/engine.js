...
export const APP_UPDATED = "2026-06-27";
...
export function smartName(name, zoom) {
  if (!name) return "";
  const large = isLargeZoom(zoom);
  if (!large) return name;
  const parts = name.trim().split(/\s+/);
  if (parts.length < 2) return name;
  // At large zoom always abbreviate last name
  const abbreviated = `${parts[0]} ${parts[parts.length - 1][0]}.`;
  // If first name itself is >10 chars or result still >12, go initials
