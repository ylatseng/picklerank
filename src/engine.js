...
export async function saveState(state) {
  try {
    ...
    // Mirror to localStorage cache on successful save
    writeCache(state);
  } catch (e) {
    // Offline or error — mirror to localStorage anyway so cache stays current
    writeCache(state);
    console.warn("saveState: Firestore unavailable, cached locally:", e.message);
  }
}

export async function loadState() {
  // Try Firestore with 8-second timeout
  try {
    const docRef = doc(db, "picklerank", DB_DOC_ID);
    ...
    // Offline or timeout — use localStorage cache
...
export async function syncPendingMatches() {
  ...
  try {
    ...
    await setDoc(docRef, { matches: JSON.stringify(merged) }, { merge: true });
    clearPendingMatches();
