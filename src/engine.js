...
export function validatePickleballScore(a, b, winTo = 11, winBy = 2) {
  if (a == null || b == null || isNaN(a) || isNaN(b)) return null;
  const high = Math.max(a, b), low = Math.min(a, b);
  if (high === winTo && high - low >= winBy) return { winner: a > b ? 0 : 1 };
  if (high > winTo && high - low === winBy) return { winner: a > b ? 0 : 1 };
  return null;
}

export function replayAllMatches(players = [], matches = []) {
  const ratingSinglesMap = {}, ratingDoublesMap = {};
  const historySinglesMap = {}, historyDoublesMap = {};
  
  players.forEach(p => {
    const base = p.baseRating ?? DEFAULT_RATING;
    ratingSinglesMap[p.id] = base;
    ratingDoublesMap[p.id] = base;
    historySinglesMap[p.id] = [{ rating: base, date: p.joinedDate || new Date().toISOString() }];
    historyDoublesMap[p.id] = [{ rating: base, date: p.joinedDate || new Date().toISOString() }];
  });
  
  const orderedMatches = [...matches].sort((a, b) => new Date(a.date) - new Date(b.date)).map(m => ({...m}));
  
  orderedMatches.forEach(match => {
    const t1ids = match.teams?.[0] || [], t2ids = match.teams?.[1] || [], allIds = [...t1ids, ...t2ids];
    const isSingles = match.type === "singles";
    const activeMap = isSingles ? ratingSinglesMap : ratingDoublesMap;
    const activeHistMap = isSingles ? historySinglesMap : historyDoublesMap;
    
    const snap = {};
    allIds.forEach(id => { snap[id] = activeMap[id] ?? DEFAULT_RATING; });
    
    const avg = ids => ids.reduce((s,id) => s + (activeMap[id] ?? DEFAULT_RATING), 0) / Math.max(1, ids.length);
    const rA = avg(t1ids), rB = avg(t2ids);
    const exp0 = calcExpected(rA, rB), act0 = match.winnerTeam === 0 ? 1 : 0;
    const margin = calcScoreMargin(match.team1Wins, match.team2Wins);
    const deltas = {};
    
    allIds.forEach(id => {
      const isT1 = t1ids.includes(id);
      const exp = isT1 ? exp0 : 1 - exp0, act = isT1 ? act0 : 1 - act0;
      const old = activeMap[id] ?? DEFAULT_RATING;
      const nr = updateRating(old, exp, act, margin);
      deltas[id] = nr - old;
      activeMap[id] = nr;
      
      if (!activeHistMap[id]) activeHistMap[id] = [];
      activeHistMap[id].push({ rating: nr, date: match.date });
    });
    
    match.ratingSnaps = snap;
    match.ratingDeltas = deltas;
  });
  
  const derivedPlayers = players.map(p => ({
    ...p,
    ratingSingles: ratingSinglesMap[p.id] ?? DEFAULT_RATING,
    ratingDoubles: ratingDoublesMap[p.id] ?? DEFAULT_RATING,
    ratingHistorySingles: historySinglesMap[p.id] ?? [{ rating: p.baseRating ?? DEFAULT_RATING, date: new Date().toISOString() }],
    ratingHistoryDoubles: historyDoublesMap[p.id] ?? [{ rating: p.baseRating ?? DEFAULT_RATING, date: new Date().toISOString() }],
  }));
