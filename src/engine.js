...
export const APP_UPDATED = "2026-06-21";

export const RELEASES = [
  {
    version: "2.0.0",
    date: "2026-06-22",
    title: "Advanced Analytics, Automated Matchmaking & System Overhaul",
    changes: [
      "Added Point Win % tracking and Form Indicators (W/L/W) for quick-glance performance analysis.",
      "Implemented a DUPR-style Rating Confidence score (📊) and Rating Volatility metric.",
      "Introduced Personal Goals for players to set and track private target ratings.",
      "Created an automated Balanced Team Suggester to calculate the fairest pairings for sessions.",
      "Added a Doubles Partner Matrix to map win rates and chemistry with every possible partner.",
      "Built an automatic Rematch Detector to flag repeated matchups in the History tab.",
      "Introduced shareable Session Summaries highlighting MVP, Most Improved, and rating deltas.",
      "Added Dashboard highlights featuring Match of the Day and a rolling 30-day Player of the Month leaderboard.",
      "Refactored Event invitations with a clean, searchable native multi-select dropdown.",
      "Isolated 'Starred Players' logic to per-user local storage, preventing leaks across shared devices.",
      "Patched real-time online presence to immediately and accurately reflect the active user.",
      "Secured deletion logic: Only Admins can now soft-delete and restore Events and History records.",
      "Expanded the Legends page with deep-dive ELO formula breakdowns and full Chinese translations."
    ]
  },
  {
    version: "1.2.1",
...
export function computePartnerMatrix(matches) {
  ...
  (matches || []).filter(m => m.type === 'doubles' && m.teams?.length === 2).forEach(m => {
    m.teams.forEach((team, ti) => {
      ...
      const won = m.winnerTeam === ti;
      for (let i = 0; i < team.length; i++) {
        for (let j = i + 1; j < team.length; j++) {
          const k = key(team[i], team[j]);
          if (!stats[k]) stats[k] = { wins: 0, total: 0 };
          stats[k].total++;
          if (won) stats[k].wins++;
        }
      }
  ...
  Object.values(stats).forEach(s => { s.pct = s.total > 0 ? Math.round(s.wins / s.total * 100) : null; });
  return stats;
}
