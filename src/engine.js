...
// Official USA Pickleball cap: matches are one game, best 2-of-3, or best 3-of-5 — 5 games max.
export const MAX_GAMES_PER_MATCH = 5;

// ─── Version & Changelog ──────────────────────────────────────────────────────
export const APP_VERSION = "2.2.73";
...
export const RELEASES = [  
    ...
    version: "2.2.73",
    date: "2026-07-01",
    title: "5 Fixes: Game Cap, Note Translation, Error Clear, Trajectory, Predictor Blend",
    changes: [
      "🎮 FIX (1): Match modes now enforce the 5-game maximum per match. The '+ Add Game' button disables and shows a hint once 5 games are added. Applies to Custom match (LogMatch).",
      "🌐 FIX (2): System-generated match notes (e.g. 'King of the Court #1: Match 2 of 3') are now stored with a translation key (sysNoteKey) and variables (sysNoteVars) separately from the user's own notes (userNote). When displayed, the system prefix is translated into the viewer's current language. User-typed notes are shown as-is. Old matches fall back to displaying the stored notes string.",
      "🐛 FIX (3): Error messages in all match modes now clear as soon as the user starts fixing their score input. Fixed in LogMatch (updGame), Session (updScore), KOTC (updScore), and Tournament (updateScore).",
      "📈 FIX (4): Rating Trajectory chart in H2H mode was rendering invisible. Root cause: ratingHistorySingles/Doubles store {rating, date} objects but the chart passed them raw to Math.min() returning NaN. Fixed by extracting .rating before plotting.",
      "🔮 FIX (5): Match Predictor in H2H now blends ELO-based win probability with actual head-to-head results. With no H2H history: 100% ELO. With 5 matches: 50/50. With 10+: 40% ELO + 60% H2H record (capped). A blend label is shown in the UI.",
    ]
    },
    {
    version: "2.2.72",
    ...
    changes: [
      "🐛 FIX (CRITICAL): Blank screen error — Settings.jsx imported clearLocalCache from engine.js but engine.js didn't export it. Added clearLocalCache() to engine.js and wired it into the logout flow in Settings.jsx.",
      "🐛 FIX (CRITICAL): Removed match-count guards from App.jsx to ensure sync works on app update.",
      "🐛 FIX: Eliminated the half-second loading screen flash.",
      "🐛 FIX: Legends tab switching no longer jumps to the top. Fixed duplicate accordion IDs.",
      "🐛 FIX: Deleted players now show 'Deleted User' instead of '?' in all cards.",
    ]
    },
    {
    version: "2.2.71",
    date: "2026-07-01",
    title: "Flash Fix, Legends Tabs, Deleted User Label, Guard Removal",
    changes: [
      "🐛 FIX: Eliminated loading screen flash.",
      "✨ IMPROVED: UI responsiveness in Legends tab.",
    ]
    },
    {
    version: "2.2.70",
    date: "2026-07-01",
    title: "Dual-Device Detection Rewritten with Session UUID",
    changes: [
      "✨ IMPROVED: Dual-device detection rewritten to use session UUIDs instead of timestamp comparison to improve reliability.",
    ]
    },
    {
    version: "2.2.69",
    date: "2026-07-01",
    title: "Fix: Data Wipe After App Update — Both Match-Count Guards Removed",
    changes: [
      "🐛 FIX: Root cause of data appearing wiped after an app update identified and fixed. App.jsx had TWO separate match-count guards which were blocking snapshots incorrectly.",
    ]
    },
    {
    version: "2.2.68",
    date: "2026-06-30",
    title: "Fix: Vite Fast Refresh Error (usePersistentFormState)",
    changes: [
      "🐛 FIX: Vite's Fast Refresh (HMR) was causing issues with usePersistentFormState export.",
    ]
    },
    {
    version: "2.2.67",
    ...
    version: "2.2.65",
    date: "2026-06-30",
    title: "Version Numbering Correction",
    changes: [
      "🔢 VERSION FIX: Two builds were both labeled v2.2.64 — this build clarifies the correct sequence. Contains all fixes from both: real-time Firestore sync via onSnapshot, data-wipe protection (writeCache removed from snapshot callback), and match-count guard preventing state rollback.",
    ]
    },
    {
    version: "2.2.64",
    date: "2026-06-30",
    title: "Restored Real-Time Sync & Fixed Data Wipe Risk",
    changes: [
      "🔄 RESTORED: Live cross-device sync is back. The app now uses Firestore's onSnapshot listener instead of a one-time getDoc fetch on load. Changes made on any device appear on all connected devices instantly without refreshing.",
      "🐛 FIX: Critical data-wipe risk removed. The onSnapshot callback was calling writeCache(), which could overwrite localStorage with an older Firestore snapshot that arrived before a pending saveState() write had completed — erasing locally-added matches and players. writeCache() is now called only by saveState(), which owns the cache and always writes synchronously before any Firestore call.",
      "🛡️ FIX: Added a match-count guard in the snapshot merge. A Firestore snapshot can never roll state backwards — if the incoming snapshot has fewer matches than the current in-memory state, it is ignored. This prevents race conditions during concurrent writes.",
    ]
    },
    {
    version: "2.2.63",
    date: "2026-06-30",
    title: "Today's Players: Sort Order, Sync & Selected-First",
    changes: [
      "✨ IMPROVED: Today's Players panel (Matches tab & QuickLog) now sorts in the correct order: selected players → starred → alphabetical. Previously used raw ELO-rating order.",
      "✨ IMPROVED: Selected players automatically rise to the top of the list when toggled, making it easy to see who's in at a glance.",
      "✨ IMPROVED: Today's Players is now fully synced across the entire app in real time. Selecting players in QuickLog immediately updates the Matches tab panel, and vice versa — no page reload needed.",
      "⭐ IMPROVED: Starred (favorited) players now show a ★ prefix in the chip grid so they're visually distinct from the rest.",
    ]
    },
    {
    version: "2.2.62",
    date: "2026-06-30",
    title: "In-App Notifications: Event Banner, RSVP Nudge & SW Re-registration",
    changes: [
      "⚡ FIX (A): SW notification timers are now re-registered on every app open. Previously, closing the browser killed the service worker's setTimeout timers, so 1-hour event reminders would never fire after the browser was closed and reopened. Now, whenever the app loads and notification permission is granted, all upcoming events are re-scheduled with the SW — keeping reminders alive as long as the user opens the app at least once that day.",
      "🔔 NEW (B): In-app event banner — when an event is 60 minutes or less away, a dismissible amber banner appears at the top of the screen (🏓 Event title — N min away · Venue). Works on all platforms including iOS where push notifications can't fire when the app is closed. Checked every minute while the app is open. Dismissed per-event and doesn't re-appear after tapping ✕.",
      "📅 NEW (C): RSVP nudge — when a logged-in player opens the app and has an upcoming event they're invited to but haven't responded to, a subtle card appears at the top with the event name and an 'RSVP →' button that navigates directly to the Events tab. Dismissed per-event. Doesn't show while the event banner is active.",
    ]
    },
    {
    version: "2.2.61",
    date: "2026-06-30",
    title: "PlayerPicker Today's Filter & QuickLog Collapsible",
    changes: [
      "✨ IMPROVED: Every PlayerPicker dropdown now shows Today's Players in a dedicated section at the top (labeled 👥 Today's Players), separated from the rest of the roster by a divider. The rest of the roster is hidden by default and can be revealed with a single tap — keeping the picker clean for normal sessions while still allowing anyone to be selected.",
      "✨ IMPROVED: Search mode bypasses the split — typing in the search bar always searches the full roster as usual.",
      "✨ IMPROVED: QuickLog's Today's Players checklist is now collapsible, matching the panel in the Matches tab. Auto-starts expanded if no players are selected yet, collapsed otherwise. Shows a name summary when collapsed.",
    ]
    },
    {
    version: "2.2.60",
    date: "2026-06-30",
    title: "Today's Players Panel in Matches Tab & KOTC Layout Fix",
    changes: [
      "✨ NEW: A collapsible 'Today's Players' panel now lives at the top of every Matches tab mode (Custom, Session, KOTC, Tournament, Compare). Tap to expand, tick who's on court today — those players float to the top of every PlayerPicker dropdown in all modes automatically. Syncs with the QuickLog checklist via the same sessionStorage key. Collapses back to a compact summary chip when closed.",
      "🐛 FIX: KOTC player selection fields were cramped into the left half of the screen due to a redundant nested grid container. Fixed to a single full-width 2-column grid — all 4 pickers now distribute evenly across the screen without overflow.",
    ]
    },
    {
    version: "2.2.59",
    date: "2026-06-30",
    title: "Today's Players Priority in All Dropdowns",
    changes: [
      "✨ IMPROVED: Players selected in Quick Log's 'Today's Players' checklist now float to the top of every PlayerPicker dropdown across the app — LogMatch (singles & doubles), Session, King of Court, Tournament, and Compare. Sort order is: Today's Players → Starred → Alphabetical. No new UI anywhere; the single QuickLog checklist sets the priority for the whole session automatically.",
    ]
    },
    {
    version: "2.2.58",
    date: "2026-06-30",
    title: "Rematch Button Removed & Image-Based Session Share",
    changes: [
      "🧹 REMOVED: The 🔄 Rematch action button has been fully removed from History match cards — the button, its handler in App.jsx, and its prop chain through History → MatchCard are all gone. The 🔁 same-day Rematch badge (passive detection, no tap action) is unchanged.",
      "✨ IMPROVED: Session Summary '📤 Share Recap' now generates and shares an actual PNG image instead of plain text — includes MVP, Most Improved, Total Points, and a clean per-round score breakdown, all dark-themed for readability in group chats. Uses the native share sheet on mobile (with the image file attached) and falls back to a direct PNG download on desktop.",
    ]
    },
    {
    version: "2.2.57",
    date: "2026-06-30",
    title: "Tournament Grid, Game Cap, DUPR Layout & Event Rename",
    changes: [
      "🐛 FIX: Round Robin with 5-6 teams no longer overflows — Tournament Teams grid now always stacks in 2 columns (matching 3-4 team layout) instead of cramming into 3 columns at higher team counts.",
      "🛡️ NEW: Matches are now capped at 5 games, matching the official USA Pickleball rule (one game, best 2-of-3, or best 3-of-5). The '+ Add Game' button disables past 5 with an explanatory note.",
      "✨ IMPROVED: History match cards now use a DUPR-style stacked layout — each player's name on its own line (instead of joined with '&'), with a divider between teams, fixing long-name overflow on doubles matches and matching the visual style of DUPR's scorecards.",
      "🐛 FIX: History match score columns now shrink responsively for matches with 5+ games (including legacy matches logged before the new cap), with a horizontal-scroll safety net so scores never push past the card edge.",
      "🌐 RENAMED: 'New Session' / 'Edit Session' in Events is now 'New Event' / 'Edit Event' in all three languages, matching existing Event terminology used elsewhere in the tab.",
      "🧹 CLEANUP: Removed a fully dead 'share match' code path (function defined in History, passed to MatchCard, but never rendered as a button) left over from a previous removal.",
    ]
    },
    {
    version: "2.2.56",
    date: "2026-06-30",
    title: "Tournament Layout Fix & PlayerPicker Keyboard Redesign",
    changes: [
      "🐛 FIX: Tournament Teams setup screen — player name fields no longer overflow off-screen with longer names. Root cause was CSS grid tracks (1fr) lacking a minimum-width override; switched to minmax(0,1fr) and added minWidth:0 to flex/grid children across Tournament Teams, LogMatch (singles & doubles), SessionMode, KingOfCourt, and Compare so every PlayerPicker container can shrink to fit the screen.",
      "✨ IMPROVED: PlayerPicker no longer auto-focuses the search field on open — the bottom sheet now opens straight to the player chip grid with the keyboard closed, so it's immediately usable without anything being blocked.",
      "✨ IMPROVED: PlayerPicker search now uses the visualViewport API — if the keyboard is opened (by tapping search), the sheet dynamically resizes to fit entirely above it instead of being covered, so the player grid stays visible and tappable while typing.",
    ]
    },
    {
    version: "2.2.55",
    ...
    },
  {
    ...
    changes: [
      "⚡ PERF: Memoized ThemeCtx.Provider to prevent app-wide re-render loops caused by object literal recreation in App render body.",
      "🐛 FIX: Restrained unbounded useEffect in App.jsx that was firing on every render; added correct dependency array.",
      "🔔 FIX: Implemented service worker cancellation protocol; ghost push notifications for deleted or rescheduled events are now successfully aborted.",
      "🛡️ FIX: Added defensive checks in Events.jsx for missing dates and corrupted player names to prevent silent runtime crashes.",
    ]
  },
  {
    version: "2.2.52",
    date: "2026-06-29",
    title: "Round Card Redesign — Names Left, Scores Right",
    changes: [
      "🎨 REDESIGN: Session and KOTC round cards completely redesigned. New layout: team names stacked vertically on the LEFT (T1 on top, 'vs' in middle, T2 below), compact score inputs on the RIGHT. Names get flex:1 + text-overflow:ellipsis so they always fit. Winner name is highlighted in accent color with ✓ prefix. Invalid scores show ⚠ indicator. This replaces the previous layout where team names were side-by-side on top and score inputs were centered below — unreadable at large font on small screens.",
      "🧩 ARCHITECTURE: Extracted shared RoundCard component used by both SessionMode and KingOfCourt. Eliminates duplicated layout code. RoundCard accepts t1Name, t2Name, t1Score, t2Score, onChange handlers, winTo/winBy, and highlight state.",
      "🔍 App scan: 0 critical issues found. Translation keys complete. No Hook-in-IIFE violations. No direct Firestore bypasses. TW block clean.",
    ]
  },
  {
    version: "2.2.51",
    date: "2026-06-29",
    title: "Session/Tournament UX Fixes & Translations",
    changes: [
      "🐛 FIX: Custom match Game Scores — X delete buttons were overflowing outside the frame. Reduced to 11px font, 3/7px padding, flexShrink:0.",
      "🐛 FIX: Session mode — error message ('Round 1: please enter valid scores') persisted even after user corrected the scores. Added useEffect watching roundScores, sessionIds, winTo, winBy: when all 3 rounds have valid scores and 4 players are selected, setErr('') clears automatically.",
      "🐛 FIX: Session, KOTC, and Tournament modes now scroll to top after successfully logging matches, so users see the results/summary immediately.",
      "🌐 TRANSLATION: 'Series:' label, 'Best of 3', 'Best of 5', 'Single' buttons, 'Series' score display in Custom match — all now use t() keys.",
      "🌐 TRANSLATION: 'Round 1:', 'Round 2:', 'Round 3:' error messages in Session mode now use t('round_lbl') → '第' in Chinese.",
      "🌐 TRANSLATION: 'SCORE PREVIEW' in Session mode now uses t('score_preview') → '比分預覽' (TW) / '比分预览' (CN).",
      "🎨 REDESIGN: Score Preview rows now stack vertically — round label on top line, team names + score on second line. Much more readable on small screens at Large zoom.",
      "🎨 REDESIGN: Team Suggester now shows T1 and T2 on separate lines with names truncating via text-overflow:ellipsis instead of wrapping. Rating averages shown on the right. Readable at all font sizes.",
    ]
  },
  {
    version: "2.2.50",
    date: "2026-06-29",
    title: "Large Font Overflow Fixes — Rankings, Roster, Welcome, QuickLog",
    changes: [
      "🐛 FIX: Rankings — P/C/D badge removed from LeaderboardRow. At Large zoom the badge took up 30px leaving no space for the player name. Badge was also not essential for daily use. Name now gets flex:1 + minWidth:0 + text-overflow:ellipsis so it always fills available space and truncates gracefully.",
      "🐛 FIX: Roster (Players view) — P/C badge and D (DUPR) badge removed from roster rows. Streak icons, lock icon, online dot, and edit button remain but are flexShrink:0. Name gets flex:1 + minWidth:0. Row uses flexWrap:nowrap with overflow:hidden so nothing pushes below the row.",
      "🐛 FIX: Welcome page overflowing at Large zoom. maxWidth:400*z scaled up to 460px (wider than a 390px phone). Fixed to maxWidth:'min(400px, 100%)' + padding capped with Math.min() + boxSizing:border-box.",
      "🐛 FIX: QuickLog score stepper (+ number − column) had fontSize:52*z (59px at Large zoom) and button width 54*z (62px). All capped with Math.min() to stay within screen bounds.",
      "🐛 FIX: lbInfo flex container was missing minWidth:0 — without this, overflow:hidden + text-overflow:ellipsis on children has no effect because the parent expands to content width. Added minWidth:0 + overflow:hidden to lbInfo in styles.js.",
      "🐛 FIX: Custom Match Game Scores row overflowing at Large zoom. scoreInput changed from minWidth:f(60)+maxWidth:f(80)+flex:0 to fixed width:f(64)+smaller padding. gameRow gap reduced from f(6) to f(4). Game label minWidth capped with Math.min(). Row now fits at all zoom levels.",
      "🔒 FIX: Offline banner offline/changes messages now use t() translation keys. Loading screen uses clamp() for responsive font sizes. Sync status banners scale with theme.zoom.",
      "🧪 Test runner: avgT1/avgT2 confirmed correct (avg1/avg2) — false positive in scan.",
    ]
  },
  {
    version: "2.2.49",
...
const getSharedState = (state) => {
  const { 
    ...
    activeView,       // navigation state — purely local, never persisted to Firestore
    ...sharedData 
  } = state;
  return sharedData;
};
...
// Real-time Firestore listener — restores live sync between devices.
// Returns an unsubscribe function. Call it on component unmount.
// Parses compressed fields the same way loadState does.
export function subscribeToState(onChange) {
  ...
  const unsub = onSnapshot(docRef, (docSnap) => {
    ...
    try {
      ...
      // NOTE: do NOT call writeCache here. saveState() owns the localStorage cache
      // and always writes synchronously before any Firestore call. Writing cache
      // from a snapshot could overwrite newer local data with an older Firestore
      // snapshot that arrived before a pending saveState() write completed.
      onChange(data);
    } catch (e) {
      console.warn("subscribeToState: parse error", e);
    }
  }, (err) => {
    console.warn("subscribeToState: Firestore error", err);
  });
  return unsub;
}
...
export async function pingPresence(playerId, sessionId) {
  if (!playerId) return;
  try {
    const docRef = doc(db, "picklerank", "main_group");
    await updateDoc(docRef, { [`presence.${playerId}`]: { ts: Date.now(), sid: sessionId || "" } });
  } catch (e) {
...
export async function clearPresence(playerId) {
  ...
  try {
    const docRef = doc(db, "picklerank", "main_group");
    await updateDoc(docRef, { [`presence.${playerId}`]: { ts: 0, sid: "" } });
  } catch (e) {
    // Fails silently if offline
  }
}
