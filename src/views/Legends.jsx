...
export default function Legends({ theme }) {
  ...
  const kRows = [0,1,2,3,4,5,10,20,30].map(n => {
    const kF = n >= PROVISIONAL_MATCH_THRESHOLD ? K_FACTOR : K_FACTOR * (2 - (n / PROVISIONAL_MATCH_THRESHOLD));
    return { n, kF };
  });
    ...
    <div style={S.view}>
      ...
      <Sec title={t("legend_how_calc_sec") || "🧮 How Ratings Are Calculated"} theme={theme}>
        <div style={{fontSize:11*z, color:theme.sub, lineHeight:1.6}}>
          ...
          <div style={{marginBottom:10*z}}>{t("legend_step1_desc")}</div>

          <div style={{fontWeight:700, color:theme.text, fontSize:12*z, marginBottom:4*z}}>{t("legend_step2_title")}</div>
          <div style={{marginBottom:10*z}}>{t("legend_step2_desc")}</div>
          <div style={{background:theme.bg, border:`1px solid ${theme.border}`, borderRadius:8*z, padding:10*z, marginBottom:10*z, fontFamily:"monospace", fontSize:11*z, color:theme.accent}}>
          ...
          <div style={{marginBottom:10*z}}>
            ...
            {(t("legend_step3_k") || "Base K = {k}").replace("{k}", K_FACTOR)}
          </div>

          <div style={{fontWeight:700, color:theme.text, fontSize:12*z, marginBottom:4*z}}>{t("legend_step4_title")}</div>
          <div style={{marginBottom:6*z}}>{(t("legend_step4_desc") || "").replace("{n}", PROVISIONAL_MATCH_THRESHOLD)}</div>
          <div style={{display:"flex", flexWrap:"wrap", gap:6*z, marginBottom:10*z}}>
            {kRows.map(({n, kF}) => (
              <div key={n} style={{padding:`${3*z}px ${7*z}px`, borderRadius:6*z, fontSize:10*z,
                ...
                color: n < PROVISIONAL_MATCH_THRESHOLD ? "#f59e0b" : "#50c878", fontWeight:600}}>
          ...
          <div>{t("legend_replay_desc")}</div>
        </div>
      </Sec>

      {/* 3. MATCH vs GAME */}
      <Sec title={t("legend_match_vs_game_sec") || "🎮 Match vs Game — What Counts?"} theme={theme}>
        <div style={{fontSize:11*z, color:theme.sub, lineHeight:1.6}}>

          {/* Definitions */}
          {[
            { label: t("legend_match_def"), color: theme.accent, desc: t("legend_match_def_desc") },
            { label: t("legend_game_def"),  color: "#f0a830",    desc: t("legend_game_def_desc") },
          ...
          {/* The key rule */}
          <div style={{
            padding:`${8*z}px ${12*z}px`, borderRadius:8*z, marginBottom:12*z,
            background:"rgba(80,200,120,0.08)", border:"1px solid rgba(80,200,120,0.3)",
            fontSize:12*z, fontWeight:700, color:"#50c878", textAlign:"center"
          }}>
            {t("legend_wl_based_on")}
          </div>

          {/* Stat table */}
          <div style={{display:"flex", flexDirection:"column", gap:6*z, marginBottom:12*z}}>
            {[
              t("legend_stat_table_wl"),
              t("legend_stat_table_winpct"),
              t("legend_stat_table_streak"),
              t("legend_stat_table_rating"),
              t("legend_stat_table_ptpct"),
              t("legend_stat_table_partner"),
            ].map((row, i) => {
          ...
          {/* Score margin example */}
          <div style={{fontWeight:700, color:theme.text, fontSize:12*z, marginBottom:4*z}}>{t("legend_margin_example_title")}</div>
          <div style={{background:theme.bg, border:`1px solid ${theme.border}`, borderRadius:8*z, padding:10*z, fontSize:11*z, color:theme.sub, lineHeight:1.6}}>
            {t("legend_margin_example")}
          </div>
        </div>
      </Sec>

      {/* 4. PICKLERANK vs DUPR */}
      <Sec title={t("legend_vs_dupr_sec") || "🆚 PickleRank vs DUPR"} theme={theme}>
        <div style={{fontSize:11*z, color:theme.sub, lineHeight:1.6}}>
          <div style={{marginBottom:10*z}}>{t("legend_vs_dupr_intro")}</div>
          ...
          <div style={{borderRadius:8*z, overflow:"hidden", border:`1px solid ${theme.border}`, marginBottom:12*z}}>
            ...
            <div style={{display:"grid", gridTemplateColumns:"1fr 1fr 1fr", background:theme.bg, borderBottom:`1px solid ${theme.border}`}}>
              <div style={{padding:`${6*z}px ${8*z}px`, fontSize:10*z, fontWeight:700, color:theme.sub}}></div>
              <div style={{padding:`${6*z}px ${8*z}px`, fontSize:11*z, fontWeight:700, color:"#40a0e0", textAlign:"center"}}>{t("legend_dupr_col")}</div>
              <div style={{padding:`${6*z}px ${8*z}px`, fontSize:11*z, fontWeight:700, color:theme.accent, textAlign:"center"}}>{t("legend_pr_col")}</div>
            </div>
            {/* Rows */}
            {[
              ...
              <div key={label} style={{
                ...
                background: i % 2 === 0 ? "transparent" : theme.bg+"44"
              }}>
          ...
          {/* Confidence note */}
          <div style={{
            padding:10*z, borderRadius:8*z,
            background:"rgba(64,160,224,0.08)", border:"1px solid rgba(64,160,224,0.25)",
            fontSize:11*z, color:theme.sub
          }}>
            {t("legend_vs_note")}
          </div>
        </div>
      ...
      <Sec title={t("legend_confidence_sec") || "📊 Rating Confidence %"} theme={theme}>
        <div style={{fontSize:11*z, color:theme.sub, lineHeight:1.6}}>
          <div style={{marginBottom:8*z}}>{t("legend_conf_intro")}</div>
      ...
      <Sec title={t("legends_icons_badges")} theme={theme}>
        ...
        <ItemRow icon="📊" title={t("legend_conf_icon_title") || "📊 Rating Confidence"} desc={t("legend_conf_icon_desc")} />
        ...
        <ItemRow icon={<span style={{display:"flex",gap:3*z}}><span style={{fontSize:9*z,fontWeight:800,color:"#50c878",background:"rgba(80,200,120,0.15)",borderRadius:3*z,padding:"1px 3px"}}>W</span><span style={{fontSize:9*z,fontWeight:800,color:"#e05050",background:"rgba(224,80,80,0.15)",borderRadius:3*z,padding:"1px 3px"}}>L</span><span style={{fontSize:9*z,fontWeight:800,color:"#50c878",background:"rgba(80,200,120,0.15)",borderRadius:3*z,padding:"1px 3px"}}>W</span></span>} title={t("legend_form_title")} desc={t("legend_form_desc")} />
      </Sec>
      ...
      <Sec title={t("legend_dashboard_sec") || "📊 Dashboard Features"} theme={theme}>
      ...
      {/* 9. SESSION MODE FEATURES */}
      <Sec title={t("legend_session_sec") || "🏓 Session Mode Features"} theme={theme}>
      ...
      {/* 10. PROFILE FEATURES */}
      <Sec title={t("legend_profile_sec") || "👤 Profile Features"} theme={theme}>
      ...
      {/* 14. MATCH MODES */}
      <Sec title={t("legend_match_modes_sec") || "🎮 Match Modes"} theme={theme}>
